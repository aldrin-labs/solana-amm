...
export function test() {
  describe("set_tokens_per_slot", () => {
    ...
    const tokensPerSlot: number = 100;
    ...
    beforeEach("create farm", async () => {
      ...
      const harvestData = await farm.addHarvest({ tokensPerSlot: 10 });
    ...
    it("fail if valueFromSlot is in the past", async () => {
      ...
      const logs = await errLogs(
        farm.setTokensPerSlot(harvestMint, currentSlot - 1, tokensPerSlot, {
          admin,
        })
      );
    ...
    it("succeeds if oldest token history slot occurs before oldest snapshot slot", async () => {
      ...
      for (let i = 4; i > -1; i--) {
        expect(tokensPerSlotHistory[4 - i].value.amount.toNumber()).to.deep.eq(
          farmInfoAfter.harvests[0].tokensPerSlot[i].value.amount.toNumber()
        );
        expect(
          tokensPerSlotHistory[4 - i].at.slot.toNumber(),
