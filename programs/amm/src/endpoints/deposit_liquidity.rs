...
/// the pool token vault 6. mint the correct amount of lp tokens to the
/// user lp token wallet 7. Update the pool curve invariant value (only
/// in the case the curve is stable)
pub fn handle<'info>(
    ...
    let lp_mint_supply = TokenAmount::new(accs.lp_mint.supply);
    ...
    let DepositResult {
        ...
        .pool
        .deposit_tokens(max_amount_tokens, lp_mint_supply)?;
