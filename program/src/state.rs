...
pub trait SwapState {
    ...
    /// Address of pool token farming freeze account
    fn token_farming_freeze_account(&self) -> &Pubkey;
    ...
    /// Farming state associated with pool
    fn farming_state(&self) -> &Pubkey;
}
...
pub struct SwapV1 {
    ...
    /// Account to freeze pool tokens for the farming
    pub token_farming_freeze_account: Pubkey,
    ...
    /// Pool farming parameters, to be used in token farming reward
...
impl SwapState for SwapV1 {
    ...
    fn token_farming_freeze_account(&self) -> &Pubkey {
        &self.token_farming_freeze_account
    }
...
impl Pack for SwapV1 {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        let output = array_mut_ref![output, 0, 387];
        let (
            is_initialized,
            nonce,
            token_program_id,
            ...
            swap_curve,
            farming_state,
        ) = mut_array_refs![output, 1, 1, 32, 32, 32, 32, 32, 32, 32, 32, 64, 33, 32];
        ...
        token_farming_freeze_account.copy_from_slice(self.token_farming_freeze_account.as_ref());
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError> {
        let input = array_ref![input, 0, 387];
        ...
        let (
            ...
            swap_curve,
            farming_state,
        ) = array_refs![input, 1, 1, 32, 32, 32, 32, 32, 32, 32, 32, 64, 33, 32];
        Ok(Self {
            ...
            token_farming_freeze_account: Pubkey::new_from_array(*token_farming_freeze_account),
...
mod tests {
    ...
    const TEST_TOKEN_FREEZE: Pubkey = Pubkey::new_from_array([5u8; 32]);
    const TEST_TOKEN_A_MINT: Pubkey = Pubkey::new_from_array([6u8; 32]);
    const TEST_TOKEN_B_MINT: Pubkey = Pubkey::new_from_array([7u8; 32]);
    const TEST_POOL_FEE_ACCOUNT: Pubkey = Pubkey::new_from_array([8u8; 32]);
    const TEST_FARMING_STATE_ACCOUNT: Pubkey = Pubkey::new_from_array([9u8; 32]);
    ...
    fn swap_version_pack() {
        ...
        let swap_info = SwapVersion::SwapV1(SwapV1 {
            ...
            token_farming_freeze_account: TEST_TOKEN_FREEZE,
            ...
            farming_state: TEST_FARMING_STATE_ACCOUNT,
        });
        ...
        assert_eq!(*unpacked.token_farming_freeze_account(), TEST_TOKEN_FREEZE);
        ...
        assert_eq!(*unpacked.farming_state(), TEST_FARMING_STATE_ACCOUNT);
    }
    ...
    fn swap_v1_pack() {
        ...
        packed.extend_from_slice(&TEST_TOKEN_FREEZE.to_bytes());
        ...
        packed.extend_from_slice(&TEST_FARMING_STATE_ACCOUNT.to_bytes());

        msg!(&packed.len().to_string());
        let unpacked = SwapV1::unpack(&packed).unwrap();
        assert_eq!(swap_info, unpacked);
